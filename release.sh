...
if [ ${GITHUB_EVENT_NAME} == 'release' ]; then
  ...
  echo "Event: ${GITHUB_EVENT_NAME}"
elif [ ${GITHUB_EVENT_NAME} == 'schedule' ] && [ ${INPUT_ALLOW_SCHEDULE^^} == 'TRUE' ]; then
  echo "Event: ${GITHUB_EVENT_NAME} with allowing schedule"
else
  echo "Unsupport event: ${GITHUB_EVENT_NAME}! without allowing schedule"
  exit 1
fi

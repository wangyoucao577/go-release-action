...
RELEASE_ASSET_NAME=${BINARY_NAME}-$(basename ${GITHUB_REPOSITORY})-${INPUT_GOOS}-${INPUT_GOARCH}
if [ ! -z "${INPUT_GOAMD64}" ]; then
    RELEASE_ASSET_NAME=${BINARY_NAME}-$(basename ${GITHUB_REPOSITORY})-${INPUT_GOOS}-${INPUT_GOARCH}-${INPUT_GOAMD64}
fi
